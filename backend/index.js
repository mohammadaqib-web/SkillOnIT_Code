...
app.post("/:newUser", (req, res) => {
  ...
  arr.push(data.newUser);
  return res
    .status(201)
    .json({ message: "Data created successfully!", data: arr });
});

app.put("/", (req, res) => {
  const data = req.body;
  obj.name = data.name;
  return res.status(200).json({ message: "Data updated successfully", obj });
});
...
app.delete("/:user", (req, res) => {
  ...
  arr = arr.filter((item) => item !== data.user);
  return res
    .status(200)
    .json({ message: "User deleted successfully", data: arr });
});
